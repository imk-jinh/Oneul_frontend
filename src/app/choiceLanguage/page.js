...
import styles from './choice.module.css'; // CSS 모듈 임포트
...
function ClientHome() {
  ...
  const [countries, setCountries] = useState([
    ...
    {
      name: 'Arab World',
      flagSrc: '/Country/united_arab_emirates_icon_127873.svg',
    },
    ...
    { name: 'Bangladesh', flagSrc: '/Country/bangladesh_icon_127886.svg' },
    {
      ...
      flagSrc: '/Country/bosnia_and_herzegovina_icon_127841.svg',
    },
    ...
    {
      name: 'Czech Republic',
      flagSrc: '/Country/czech_republic_icon_127835.svg',
    },
  ...
  const handleCountryClick = (country) => {
    ...
    const contryElement = document.querySelectorAll('.CountryList');
    contryElement.forEach((element) => {
      if (element.querySelector('.Country_Title').innerText === country.name) {
        element.classList.add('hovered'); // 클릭 시 hovered 클래스 추가로 크기와 색상 변경

        // 일정 시간 후 회색 배경을 원래 색상으로 복귀
        setTimeout(() => {
          element.classList.remove('hovered');
        }, 500); // 0.5초 후 원래 색상으로 복귀
      } else {
        element.classList.remove('hovered'); // 다른 요소의 hovered 클래스 제거
      }
    });
  };
  ...
  if (!isMounted) {
    return null; // 로딩 중일 때는 null 또는 로딩 스피너를 표시
  }

  // useEffect(() => {
  //   console.log(selectedCountry); // 선택된 국가의 이름이 변경될 때마다 출력
  // }, [selectedCountry]);
  ...
  return (
    <div className={styles.frame}>
        ...
        {countries.map((country, index) => (
          ...
          >
            ...
            <div className={styles.Contry}>
              ...
              />
              <p className={styles.Country_Title}>{country.name}</p>
              {selectedCountry === country.name && (
                <img className={styles.Check} src="/etc/check.svg" alt="Check" />
              )}
            </div>
          </div>
        ))}
      ...
      <Link href="/setName" style={{ textDecoration: 'none' }}>
        <div
